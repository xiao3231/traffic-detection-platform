...
PROTOCOL_PATH = os.path.join(current_dir, 'protocol')
...
class GetGoodx():
    ...
    def handle_flow(self, obj, data):
        ...
        # 检查数据包是否有IP层，然后传递正确的参数
        if data.haslayer("IP"):
            ip_layer = data.getlayer("IP")
            if data.haslayer("TCP"):
                data3 = obj.tcp_decode(data, ip_layer)
                # 合并数据
                data_result = {**data1, **data3}
            elif data.haslayer("UDP"):
                data4 = obj.udp_decode(data, ip_layer)
                data_result = {**data1, **data4}
            else:
                data2 = obj.ip_decode(data)
                data_result = {**data1, **data2}
        elif data.haslayer("IPv6"):
            ip_layer = data.getlayer("IPv6")
            if data.haslayer("TCP"):
                data3 = obj.tcp_decode(data, ip_layer)
                data_result = {**data1, **data3}
            elif data.haslayer("UDP"):
