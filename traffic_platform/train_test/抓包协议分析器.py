...
def packet_producer():
    sniff(prn = lambda pkt: packet_consumer(pkt), stop_filter = lambda pkt: stop_sniff_event.is_set(),
          filter = fitler_entry.get(), iface = None)
...
# 该函数接收一个数据包作为参数，并对数据包进行解析、分析、存储等操作。
# 具体来说，该函数首先使用全局变量sniff_count和sniff_array来记录捕获到的数据包数量和数据包列表。
# 然后，该函数判断是否暂停捕获数据包，如果没有暂停，则对捕获到的数据包进行处理。
# 该函数使用timestamp2time函数将数据包的时间戳转换为可读的时间格式，并使用proto_names列表推导出数据包的协议类型。
# 然后，该函数根据协议类型解析出数据包的源地址、目的地址、长度和摘要信息，并将这些信息添加到一个Treeview控件中显示。
# 最后，该函数使用update_idletasks方法更新Treeview控件，以便及时显示新的数据包信息。
